...
# 이미지 파일 한 번에 불러오기
images = glob.glob('./0803_60_test(1)/1.bmp')
print(images)
calibration_flags = (
...
print("openCV rms:", round(rms, 4), file=f)
# print("Reprojection Error :", np.sqrt(sum(meanErrorTotal) / N_OK), file=f)
print(f'소요시간: {round(end-start, 2)}초', file=f)
for i in range(len(rvecs)):
    for j in range(3):
        rvecs_deg = math.degrees(rvecs[i][j])
        print(rvecs[i][j], round(rvecs_deg, 1), file=f)
    print("", file=f)
f.close()