...
# 이미지 파일 한 번에 불러오기
images = glob.glob('./fishnewcheck0712/*.bmp')
print(images)
...
# 일단 캘리브레이션 코너검출은 막아둠 -> 2d, 3d 좌표만 표시되게
...
beforeK = [ [596.86271316, 0, 775.9626677],
      [0, 594.42325289, 653.08879771],
      [0, 0, 1] ]
afterK = np.array(beforeK)
beforeD = [ [0.00196481],
     [0.01230001],
     [-0.01321082],
     [0.00404112] ]
afterD = np.array(beforeD)
...
for idx in range(N_OK):
    ...
    for i in range(CHECKERBOARD[0] * CHECKERBOARD[1]):
        ...
        # imgpoints2 = cv2.fisheye.projectPoints(objp, rvecs[idx], tvecs[idx], K, D)  # K,D 는 1개값 r, t 는 이미지개수만큼..
        corner_reprojection = imgpoints2[0][corner_index]
        # corner_reprojection = imgpoints2[0][0][corner_index]
    ...
    imgpoints2 = imgpoints2.reshape(90, 1, 2)
    ...
    for j in range(CHECKERBOARD[0] * CHECKERBOARD[1]):
        cv2.circle(img, (int(imgpoints[idx][j][0][0]), int(imgpoints[idx][j][0][1])), 10, (255,0,255), -1)
        cv2.circle(img, (int(imgpoints2[j][0][0]), int(imgpoints2[j][0][1])), 8, (0,255,0), -1)
    # cv2.imwrite(f'./after/cameramove_23{idx}.jpg', img)  # 이미지 저장용
