...
for idx in range(N_OK):
    ...
    # print(f'{idx} 번째 reprojection error: {mean_error}') # 사진 한 장당 reprojection error
    ...
    for j in range(checkboard_size):
        cv2.circle(circle_img, (int(imgpoints[idx][j][0][0]), int(imgpoints[idx][j][0][1])), 10, (255, 0, 255),
                   -1)  # 2d points (pink)
        cv2.circle(circle_img, (int(re_imgpoints[j][0][0]), int(re_imgpoints[j][0][1])), 8, (0, 255, 0),
                   -1)  # reprojected 3d points (green)
